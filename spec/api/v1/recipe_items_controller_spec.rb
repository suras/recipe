...
describe "Recipe Items API", :type => :api do
  ...
  it 'shows a recipe items' do
    ...
    get "/api/v1/recipe_items/", {id: @recipe_item.id.to_s}  
    expect(response).to be_success
    expect(json["_id"]['$oid']).to eq(@recipe_item.id.to_s) 
  end

  it 'updates a recipe items' do
    @recipe_item = @user.recipe_items.create(name:"test")
    params = {recipe_item: {name: "updated"}, id: @recipe_item.id.to_s}
    patch "/api/v1/recipe_items/", params, authorization: @authorization  
    expect(response).to be_success
  ...
  it 'delete a recipe items' do
    @recipe_item = @user.recipe_items.create(name:"test")
    expect {
      delete "/api/v1/recipe_items/", {id: @recipe_item.id.to_s}, authorization: @authorization  
